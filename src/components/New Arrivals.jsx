...
const ProductCard = ({ product, onShowDetails }) => {
  ...
  return (
    <div className="relative bg-gray-50 rounded-lg p-3 sm:p-6 flex flex-col items-center">
      <div className="relative group">
        <img
          src={product.image}
          alt={product.title}
          className="w-[150px] h-[150px] sm:w-[200px] sm:h-[200px] md:w-[250px] md:h-[250px] object-contain mb-2 sm:mb-4 bg-[#eff2fd] rounded-lg"
        />
        <div className="cursor-pointer absolute inset-0 flex items-end justify-between px-2 pb-4 sm:pb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button 
            onClick={() => onShowDetails(product)}
            className="w-7 h-7 sm:w-9 sm:h-9 flex items-center justify-center rounded bg-white shadow hover:bg-gray-100"
          >
            <FaEye className="text-gray-600 text-sm sm:text-base" />
          </button>
          <button 
            onClick={handleAddToCart}
            className="px-2 sm:px-4 h-7 sm:h-9 flex items-center justify-center rounded bg-blue-600 text-white text-xs sm:text-sm font-medium hover:bg-blue-700"
          >
            Add to cart
          </button>
          <button 
      ...
      <p className="font-medium text-sm sm:text-base ease-out duration-200 hover:text-blue-800 mb-1 sm:mb-1.5 cursor-pointer text-center">
        {product.title}
      </p>
      <div className="flex items-center gap-1 sm:gap-2 font-medium text-base sm:text-lg">
        {product.price}
        <span className="line-through text-gray-400 font-normal text-sm sm:text-base ml-1 sm:ml-2">
          {product.oldPrice}
        </span>
      </div>
    </div>
...
const NewArrivals = () => {
  ...
  return (
    <div className="bg-white text-gray-900 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-10">
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        <div className="flex items-center space-x-2 text-gray-600 text-xs sm:text-sm font-medium">
        ...
        <button 
          ...
          className="text-xs sm:text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded px-4 sm:px-7 py-2 sm:py-2.5 hover:bg-[#0f1f3a] hover:text-white transition duration-300 ease-in-out cursor-pointer"
        >
          View All
        </button>
      </div>
      <h2 className="font-bold text-2xl sm:text-3xl xl:text-heading-5 leading-8 text-gray-800 mb-4">
        New Arrivals
      </h2>
          ...
          <>
            ...
            >
              <div className="min-h-screen flex items-center justify-center p-4">
                <motion.div
                  className="relative w-full max-w-5xl bg-white rounded-lg"
